      ...
      <div className="flex flex-1 items-center justify-center px-8">
        <div className="w-full max-w-sm">
          ...
          <div className="rounded-2xl border p-7 shadow-sm" style={{ background: "white", borderColor: "var(--border)" }}>
            {/* Google OAuth */}
            <button type="button"
              onClick={() => signIn("google", { callbackUrl: invite ? `/invite/${invite}` : "/dashboard" })}
          ...
          <p className="text-center text-sm mt-5" style={{ color: "var(--text-muted)" }}>
            ...
            <Link href={invite ? `/register?invite=${invite}` : "/register"} className="font-semibold" style={{ color: "var(--gold)" }}>
              Create one free
            </Link>
          </p>
        </div>
